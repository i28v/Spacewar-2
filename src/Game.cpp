...
void Game::init()
{
	...
	{
		starSize = rand() % 2 + 1;
		backgroundStars.push_back(new Star({ static_cast<float>(rand() % 800), static_cast<float>(rand() % 600) }, { static_cast<float>(starSize), static_cast<float>(starSize) }, Color::White));
...
Game::~Game()
{
	...
	{
		backgroundStars.erase(backgroundStars.begin());
	}
}
...
void Game::update()
{
	...
	*/
	starSize = rand() % 2 + 1;
	backgroundStars.push_back(new Star({ static_cast<float>(rand() % 800), static_cast<float>(rand() % -1) }, { static_cast<float>(starSize), static_cast<float>(starSize) }, Color::White));
	for (int i = 0; i < backgroundStars.size(); i++)
	{
